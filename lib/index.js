...
function listFilesRelative(dir) {
  return listFiles(dir).then(
    absolutePaths => absolutePaths.map(absolutePath => relative(dir, absolutePath))
  );
}
...
export default class SassDir extends Plugin {
  ...
  renderDir(inputPath, outputPath) {
    return listFilesRelative(inputPath).then(
      relativePaths => this.renderFiles(
        inputPath,
        relativePaths.filter(this.filterFile.bind(this)),
        outputPath
      )
    );
  }
