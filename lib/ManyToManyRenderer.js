...
export default class ManyToManyRenderer extends Plugin {
  ...
  renderFiles(relativePaths) {
    return resolve(relativePaths).map(relativePath => {
      return this.render(this.inputPaths[0], relativePath)
      .then(renderOutput => {
        renderOutput.relativePath = relativePath;
        return renderOutput;
      });
    });
  }
  ...
  // abstract renderFile(inputPath, relativePath)
  // => {
  //   includedFiles: [
  //     // relative paths to inputPath
  //   ],
  //   outputFiles: [
  //     {
  //       path: <relative path>,
