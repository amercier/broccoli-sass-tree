...
export default class ManyToManyRenderer extends Plugin {
  ...
  build() {
    ...
    const changedFiles = this.listFilesToRender();
    return changedFiles
  ...
  listFilesToRender() {
    ...
    .then(() => this.dependencyGraph.getDirtyNodes());
  }
  ...
  dirtyDependencyGraph() {
    const updatedFiles = this.fileStatsCache.listUpdatedFiles();
    return updatedFiles.map(({ relativePath }) => {
